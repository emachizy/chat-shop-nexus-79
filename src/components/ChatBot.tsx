...
import { X, Send, ShoppingCart, MessageCircle, CreditCard, Check } from 'lucide-react';
...
const ChatBot = ({ isOpen, onClose, products, onAddToCart }: ChatBotProps) => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      type: 'assistant',
      content: "Hi! I'm your AI shopping assistant. You can ask me to find products, add items to cart, checkout, and even process payments - all through our conversation! Try: 'show me phones', 'specs of iPhone', 'add iPhone to cart', 'checkout', or 'pay now'.",
      timestamp: new Date(),
    }
  ]);
  ...
  const parseCommand = (input: string) => {
    ...
    if (lowercaseInput.includes('spec') || lowercaseInput.includes('detail') || 
        lowercaseInput.includes('about') || lowercaseInput.includes('tell me more')) {
      // Extract product name from the query
      const words = input.split(' ');
      const productKeywords = words.filter(word => 
        !['spec', 'specs', 'specification', 'specifications', 'detail', 'details', 
         'about', 'tell', 'me', 'more', 'show', 'what', 'is', 'the', 'of'].includes(word.toLowerCase())
      );
    ...
    if (lowercaseInput.includes('add') && lowercaseInput.includes('cart')) {
      const productName = input.toLowerCase().replace(/add|to|cart/g, '').trim();
    ...
    if (lowercaseInput.includes('checkout') || lowercaseInput.includes('check out')) {
    ...
    if (lowercaseInput.includes('cart') || lowercaseInput.includes('show cart')) {
  ...
  const addToCartInChat = (product: Product) => {
    ...
    const confirmMessage: ChatMessage = {
      ...
      type: 'assistant',
      content: `✅ Added ${product.name} to your cart! You now have ${existingItem ? existingItem.quantity + 1 : 1} item(s). Say "checkout" when you're ready to proceed.`,
      timestamp: new Date(),
    };
  ...
  const callOpenAIWithMemory = async (userInput: string, foundProducts: Product[] = []) => {
    // Add user message to memory
    const newMemory = [...conversationMemory, { role: 'user' as const, content: userInput }];
    
    // Add context about found products if any
    const contextMessage = foundProducts.length > 0 
      ? `Found ${foundProducts.length} products matching the user's query. Products: ${foundProducts.map(p => p.name).join(', ')}`
      : '';

    const systemContext = contextMessage ? 
      `${newMemory[0].content} ${contextMessage}` : newMemory[0].content;

    try {
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer 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',
      ...
      if (response.ok) {
        ...
        const updatedMemory = [...newMemory, { role: 'assistant' as const, content: assistantResponse }];
    ...
    try {
      ...
      switch (command.type) {
        case 'specifications':
          ...
          if (specProduct) {
            ...
            const errorMessage: ChatMessage = {
              id: Date.now().toString(),
              type: 'assistant',
              content: "I couldn't find specifications for that product. Try asking about a specific product name or browse our available products first.",
              timestamp: new Date(),
            };
            setMessages(prev => [...prev, errorMessage]);
          }
        ...
        case 'add_to_cart':
          if (command.product) {
            ...
            const errorMessage: ChatMessage = {
              id: Date.now().toString(),
              type: 'assistant',
              content: "I couldn't find that product. Try being more specific or browse available products first.",
              timestamp: new Date(),
            };
            setMessages(prev => [...prev, errorMessage]);
          }
        ...
        default:
          ...
          const assistantResponse = await callOpenAIWithMemory(currentInput, foundProducts);
